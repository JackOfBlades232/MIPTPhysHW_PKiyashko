...
class VerletMassPoint2d {
    ...
    friend class Verlet2PointsConstraint;
...
public:
    ...
    explicit VerletMassPoint2d(vec2d_t init_pos, double mass) 
        : m_mass(mass), m_pos(init_pos), m_prev_pos(init_pos) {};
...
class Verlet2PointsConstraint {
    double m_rest_length      = 0.0;
    double m_relaxation_coeff = 1.0;

public:
    Verlet2PointsConstraint() = default;
    explicit Verlet2PointsConstraint(double rest_length, double relaxation_coeff)
        : m_rest_length(rest_length), m_relaxation_coeff(relaxation_coeff) {}

    // @SPEED(PKiyashko): this is not optimized at all (verbose & w/ sqrt), and could be if it will be needed.
